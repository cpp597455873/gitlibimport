...
describe Git::BranchPushService, services: true do
  ...
  describe "cross-reference notes" do
    ...
    it "creates a note if a pushed commit mentions an issue", :sidekiq_inline_tech_debt do
    ...
    it "defaults to the pushing user if the commit's author is not known", :sidekiq_inline_tech_debt do
      allow(commit).to receive_messages(
    ...
    it "finds references in the first push to a non-default branch", :sidekiq_inline_tech_debt do
  ...
  describe "issue metrics" do
    ...
    context "while saving the 'first_mentioned_in_commit_at' metric for an issue" do
      it 'sets the metric for referenced issues', :sidekiq_inline_tech_debt do
        execute_service(project, user, oldrev, newrev, ref)
  ...
  describe "closing issues from pushed commits containing a closing reference" do
    ...
    context "to default branches" do
      it "closes issues", :sidekiq_inline_tech_debt do
        execute_service(project, commit_author, oldrev, newrev, ref)
        expect(Issue.find(issue.id)).to be_closed
      end

      it "adds a note indicating that the issue is now closed", :sidekiq_inline_tech_debt do
    ...
    context "to non-default branches" do
      ...
      it "creates cross-reference notes", :sidekiq_inline_tech_debt do
    ...
    context "for jira issue tracker" do
      ...
      context "mentioning an issue" do
        ...
        it "initiates one api call to jira server to mention the issue", :sidekiq_inline_tech_debt do
          execute_service(project, user, oldrev, newrev, ref)
      ...
      context "closing an issue" do
        ...
        context "using right markdown", :sidekiq_inline_tech_debt do
        ...
        context "using internal issue reference" do
          ...
          context 'when internal issues are enabled', :sidekiq_inline_tech_debt do
            let(:issue) { create(:issue, project: project) }
